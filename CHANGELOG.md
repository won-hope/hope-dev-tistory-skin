...
## [2026-06-29] V9.1.0 "Compact & Cohesive Post Footer"
- **포스트 하단 컴팩트 뷰 적용**: 포스트 하단의 태그, 이전/다음글, 프로필 영역을 애플리케이션 느낌의 작고 단단한 단일 열(Stack) 레이아웃으로 압축. 
- **거대 레거시 클래스 의존성 제거**: 하단 네비게이션 영역에서 불필요한 1:1 종횡비(aspect-ratio)와 거대한 패딩을 몰고 오던 `inpa-card` 및 `bento-card` 클래스 완전히 제거.
- **해시태그 인라인화**: 무거운 박스 형태의 해시태그를 본문과 어울리는 미니멀 인라인 텍스트로 단순화.
- **버전 및 저작권 명시 강화**: 푸터 영역에 스킨 버전(v9.1.0) 표기 추가 및 저작권 연도 2026년으로 현행화.
